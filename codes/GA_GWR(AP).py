# -*- coding: UTF-8 -*-
'''
The GA-GWR for discovering governing equation bentween terrain and annual precipitation
'''
import pickle
import numpy as np
...
def combine_slice(lat,lon):
    ...
    with open('result_save/all_slice.pkl', 'wb') as f:
        pickle.dump(all_slice, f)
    with open('result_save/all_slice_mask.pkl', 'wb') as f:
        pickle.dump(all_slice_mask, f)
    with open('result_save/coord_save.pkl', 'wb') as f:
    ...
    with open('result_save/all_slice.pkl', 'rb') as f:
        all_slice= pickle.load(f)
    with open('result_save/all_slice_mask.pkl', 'rb') as f:
    ...
    with open('result_save/concat_all_slice.pkl', 'wb') as f:
        pickle.dump(concat_all,f)
    with open('result_save/split_index.pkl', 'wb') as f:
    ...
    with open('result_save/concat_all_slice_mask.pkl', 'wb') as f:
        pickle.dump(concat_all_mask,f)
    with open('result_save/split_index_mask.pkl', 'wb') as f:
...
def combine_slice_validate(lat,lon,large_precio,small_precio,large_HT,small_HT,large_relief,small_relief):
    ...
    print(f'length of concat_all_slice_mask is:{len(concat_all)}')
class GA():
    def __init__(self, concat_all_slice,concat_all_slice_mask,split_index,split_index_mask,years,wl):
        self.max_length = 7
        ...
        self.var_num=self.concat_X_slice.shape[1]-self.years.shape[0]
    ...
    def translate_DNA(self, gene):
        ...
        self.X = self.concat_X_slice[:, self.years.shape[0]:]
        self.X_mask = self.concat_X_slice_mask[:, self.years.shape[0]:]
        self.y = self.concat_X_slice[:, 0:self.years.shape[0]]
        self.y_mask = self.concat_X_slice_mask[:, 0:self.years.shape[0]]
...
if __name__ == '__main__':
    ...
    #         all_year_precip=ma.concatenate((all_year_precip,precip_year),axis=1)
    # mean_precio=all_year_precip.reshape(184,288,years.shape[0])
    mean_precio=pickle.load(open('Dataset/Dataset_precio_train.pkl', 'rb'))
    print(mean_precio.shape)
    ...
    print(large_precio,small_precio)
    large_HT=ma.max(HT)
    small_HT=ma.min(HT)
    ...
    if MODEL == 'GA_GWR':
        ...
        with open('result_save/all_slice_mask.pkl', 'rb') as f:
            all_slice_mask = pickle.load(f)
        with open('result_save/concat_all_slice.pkl', 'rb') as f:
            concat_all_slice = pickle.load(f)
        with open('result_save/split_index.pkl', 'rb') as f:
            split_index = pickle.load(f)
        with open('result_save/concat_all_slice_mask.pkl', 'rb') as f:
            concat_all_slice_mask = pickle.load(f)
        with open('result_save/split_index_mask.pkl', 'rb') as f:
            split_index_mask = pickle.load(f)


        print('=============Start GA====================\n')
        optimizer = GA(concat_all_slice,concat_all_slice_mask,split_index,split_index_mask,years,wl)
        print(f'The size of candidate library is: {concat_all_slice.shape[1]-years.shape[0]}')
        print(f'The epi for GA is {optimizer.epi}')

        optimizer.evolution(print_info=True)
    if MODEL=='validate':
        years = np.arange(2011, 2016, 1)
        combine_slice_validate(lat_num,lon_num,large_precio,small_precio,large_HT,small_HT,large_relief,small_relief)
        best_Chrom=[[0],[2],[1,2,4],[5]]
        with open('result_save/all_slice_valiate.pkl', 'rb') as f:
            all_slice = pickle.load(f)
        with open('result_save/all_slice_mask_valiate.pkl', 'rb') as f:
            all_slice_mask = pickle.load(f)
        with open('result_save/concat_all_slice_valiate.pkl', 'rb') as f:
            concat_all_slice = pickle.load(f)
        with open('result_save/split_index_valiate.pkl', 'rb') as f:
            split_index = pickle.load(f)
        with open('result_save/concat_all_slice_mask_valiate.pkl', 'rb') as f:
            concat_all_slice_mask = pickle.load(f)
        with open('result_save/split_index_mask_valiate.pkl', 'rb') as f:
            split_index_mask = pickle.load(f)
        with open('result_save/coord_save_valiate.pkl', 'rb') as f:
            coord_save = pickle.load(f)


        optimizer = GA(concat_all_slice, concat_all_slice_mask, split_index, split_index_mask,years,wl)
        gene_translate, gene_translate_mask,y,y_mask,length_penalty_coef = optimizer.translate_DNA(best_Chrom)
        MSE, R_2, MSE_total = optimizer.get_fitness()
        print(MSE,R_2,MSE_total)

        all_pred = ma.ones([lat_num,lon_num,years.shape[0]])*(-1e8)
        all_true = ma.ones([lat_num,lon_num,years.shape[0]])*(-1e8)
        all_coef=ma.ones([len(best_Chrom)+1,lat_num,lon_num,years.shape[0]])*(-1e8)
        all_coef_list=[]
        all_HT=[]
        all_relief=[]
        all_Hy=[]
        ...
        a=[]
        b=[]
        for iter in range(len(split_index) - 1):
            coord=coord_save[iter]
            lst = np.linalg.lstsq(gene_translate[split_index[iter]:split_index[iter + 1]],
                                  y[split_index[iter]:split_index[iter + 1]],
                                  rcond=None)
            coef = lst[0]
            target_mask = np.dot(gene_translate_mask[split_index_mask[iter]:split_index_mask[iter + 1]], coef)[
                int((2 * wl + 1) * (2 * wl + 1) / 2)]
            all_pred[coord[0],coord[1]]=target_mask
            all_true[coord[0],coord[1]]=y_mask[split_index_mask[iter]:split_index_mask[iter + 1]][int((2 * wl + 1) * (2 * wl + 1) / 2)]
            a.append(target_mask)
            b.append(y_mask[split_index_mask[iter]:split_index_mask[iter + 1]][int((2 * wl + 1) * (2 * wl + 1) / 2)])
            for i in range(coef.shape[0]):
                all_coef[i,coord[0],coord[1]]=coef[i]
            all_HT.append(HT[coord[0],coord[1]])
            all_relief.append(relief[coord[0], coord[1]])
            all_Hy.append(dHTdy[coord[0], coord[1]])
            all_Hx.append(dHTdx[coord[0], coord[1]]*aspect[coord[0], coord[1]])
            all_coef_list.append(coef)


        all_coef_list=np.squeeze(np.array(all_coef_list))
        all_HT=np.array(all_HT)*(large_HT-small_HT)+small_HT
        all_relief = np.array(all_relief) * (large_relief - small_relief) + small_relief


        y = np.array(a)
        target = np.array(b)
        RMSE = np.sqrt(np.mean((y - target) ** 2))
        R_2 = (1 - (((y - target) ** 2).sum() / ((y - y.mean()) ** 2).sum()))
        print(RMSE,R_2,ma.corrcoef(y.reshape(-1, ), target.reshape(-1, )))
